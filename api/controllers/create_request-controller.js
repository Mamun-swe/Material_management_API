...
exports.createRequest = function(args, res, next) {
    ...
    function _saveMatches( requestId, inputParams, matches, callback ) {
                          ...
                          sql("SELECT id FROM matched_results WHERE own_request_id = @requestId", { requestId: requestId }).then(matchedResult => {    
                              ...
                              try {
                                  sql(matchedSummaryInsertSqlQuery).then(matchedSummaryInsert => {
                                      callback( null, requestId, inputParams, matches );
                                  });
     ...
     /**
      * Saves matched requests for the other sides
      *
      * @param requestId
      * @param inputParams
      * @param matches
      ...
      * @private
      */
      function _saveOtherSideMatches( requestId, inputParams, matches, callback ) {

        console.log("STEP 11: _saveOtherSideMatches");
                ...
                sql(matchedResultInsertSqlQuery).then(matchedResultInsert => {
                    ...
                    try {
                        sql("SELECT id FROM matched_results WHERE own_request_id IN (" + matchedIds.join() + ") AND matched_request_id = @requestId", { requestId: requestId }).then(matchedResult => {
                            ...
                            //console.error("OTHER SIDE MATCHES:", JSON.stringify(matchedResult, null, 2));
